...
# Script to build static Next.js export, upload to R2, and deploy to Cloudflare Pages
echo "Starting static deployment process with R2 storage..."

# 1. Build the Next.js application in static export mode
...
if npx wrangler pages deployment create ./static-deployment \
  ...
  --branch=main \
  --commit-message="Static deployment via deploy-static-with-r2.sh script"; then
